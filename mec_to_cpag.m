function [P,P0,nRules] = mec_to_cpag(S,C,D)
  % calculates the completed Partial Ancestral Graph representation for a given 
  % Markov Equivalence Class (MEC = {S,C,D}) encoded as
  % input:
  % - S = (overloaded) skeleton as NxN 0/1 adjacency matrix OR full MEC M
  ...
  % - P0 = idem, core PAG (v-structs + one mark per triple order k>=1)
  % - nRules = array with counters of orientation rules triggered (FCI numbering, see mag_to_cpag)
  %   note: rule 15 = R4c (R4a after core PAG, R4' in GPS article p4)
  %
  % Description:
  % Follows the augmented FCI orientation strategy from [Zhang,2008] (see
  % mag_to_cpag), but now adapted to MECs. First MEC is mapped to core PAG,
  % then standard FCI orientation rules apply, with the only exception the
  % discriminating path rule R4 that now only follows the first branch (R4a,
  % noncollider), as all discriminated colliders are already covered in the
  % core PAG. 
  % Note the implementation below also follows a more efficient way to
  ...
  % Step 1: Initialise
  if (nargin == 1),
    % assume only MEC struct M was passed: extract fields
    M = S;
    S = M.S;
    C = M.C; 
    D = M.D;
  ...
  % output
  P0 = []; P = [];
  nRules = zeros(1,15);  % function global counter of rules R0b-R10 that triggered
  ...
  % M -> CorePAG P0 -> (R1-R4c) -> (R5-R6-R7) -> (R8a-R10) -> P
  P0 = Core_PAG(S,C,D);     % add v-structures, discr.path marks
  
  P = FCI_Orientation_R1_R4c(P0);   % add remaining arrowheads (+ some tails)
...
% Core PAG
% =======================================================================
function P0 = Core_PAG(S,C,D)
  % 1: skeleton with circle marks
  P0 = 3*(S > 0);
  
  % 2: process all colliders triples in C(i,:) = [k,x,z,y,q]
  for i = 1:size(C,1)
    ...
    y = C(i,4);
    if (k == 0)
      % zero order (v-structures)
      P0(z,x) = 2; 
      P0(z,y) = 2; 
      nRules(1) = nRules(1) + 1;    % R0 (v-structures in [Zhang,2008])
      if (debug > 1), fprintf('Core PAG-R0: orient %d *-> %d <-* %d\n',x,z,y); end; 
    else
      % 1+ order (discr.coll., arrowhead R4b)
      P0(z,y) = 2;   % note: we could also orient P(z,x), but this for the core PAG 
      nRules(7) = nRules(7) + 1;    % R4b (second branch of R4 in [Zhang,2008])
      if (debug > 1), fprintf('Core PAG-R4b: orient %d <-* %d\n',z,y); end; 
    end;
  end;  % for C
  
  % 3: process all noncolliders triples with order k>0 in D(i,:) = [k,x,z,y,q]
  i0 = find(D(:,1) > 0,1);  % find first noncoll. order > 0
  for i = i0:size(D,1)
    % extract relevant fields
    z = D(i,3);
    y = D(i,4);
    % process 1+ order (discr.noncoll, tail mark R4a)
    P0(z,y) = 1;    
    nRules(6) = nRules(6) + 1;    % R4a (first branch of R4)
    if (debug > 1), fprintf('Core PAG-R4a: orient %d --* %d\n',z,y); end; 
  end;  % for D
end; % subfunction Core_PAG

% =======================================================================
% Orientation rules R1-R4c
% =======================================================================
function P = FCI_Orientation_R1_R4c(P0)
  % 1: initialize
  P = P0;           % start from core PAG
  N = size(P,1);  
  ...
  while ~isempty(check_nodes)
    ...
    while ~isempty(check_nodes)    
      ...
      % ============================
      % R1: definite ancestor from zero order noncollider
      ...
      for z = check_nodes(:)'
        ...
        for x = X(:)'
          ...
          % (also allow for tail marks z --o y from core PAG)
          Y = find( (P(z,:) == 3 | (P(z,:) == 1 & P(:,z)' == 3)) & P(x,:) == 0);
          if ~isempty(Y),
            P(z,Y) = 1;
            P(Y,z) = 2;
            % add to new_arrow
    ...
    % ===================================
    % 2b: process R4c discriminating paths (R4a after core PAG)
    % ===================================
    % if (z o-* y) and disc.path=(x,...,w,z,y), then orient z --> y if z o-* y in G 
    % NOTE: This new version without explicit construction of the discr.path 
    % itself also applies to the standard FCI orientation rule R4.
    ...
    %   then for all w <-* z o-* y orient z --> y.
      ...
      for w = W(:)'
        ...
        if ~isempty(x)
          ...
          for z = Z(:)'
            ...
            % R4c: put arc at z --> y (bidirected tail already in core PAG
            P(z,y) = 1;
            P(y,z) = 2;
            % add to new_arrow (may trigger other orientations)
            new_arrow(y) = 1;
            if (debug > 1), fprintf('FCI: R4c orients %d --> %d \n',z,y); end; 
            nRules(15) = nRules(15) + 1;  % R4c = 15 
          end;  % for z
        end;  % if (x)
      end;  % for w
    end;  % for y   
...
end  % function FCI_Orientation_R1_R4c
